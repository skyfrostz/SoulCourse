<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="frontend/public/favicon.svg">
    <img alt="选科知谈" src="frontend/public/favicon.svg" width="80">
  </picture>
</p>

<h1 align="center">选科知谈 · Xuan Ke Zhi Tan</h1>
...
src/
  ...
  ├── lib/           → API client, seed data, labels, knowledge base
  ├── stores/        → Pinia stores (auth, filters, local engagement)
  ├── composables/   → TanStack Query hooks + fallback data merge
...
| **AI Choice Advice** | DeepSeek-powered personalized subject selection advice with offline fallback |
...
| **Offline-First UX** | Frontend renders with full interactivity using local seed data when the API is unavailable |
...
```bash
git clone https://github.com/your-org/subject-choice-forum.git
...
- [x] Offline-first frontend with localStorage engagement
...
**选科知谈**是一个面向中国新高考选科的全栈论坛应用，为中国高一学生和家长提供选科交流、数据洞察和 AI 辅助决策服务。

### 项目背景

每年超过一千万中国学生面临高考。在新高考改革下，选科是在高一就要做出的高利害决策——物理/历史二选一，再从化学、生物、政治、地理中选两科——这个组合直接决定大学专业报考资格。然而大多数学生和家长缺乏系统的、可信赖的指导。**选科知谈**通过将经验分享、官方数据聚合和 AI 辅助决策整合到一个平台，填补了这一信息鸿沟。
...
**選科知談**（Xuan Ke Zhi Tan）は、中国の新高考（新ガオカオ）における科目選択を支援するフルスタックフォーラムアプリケーションです。高校1年生とその保護者が、科目選択に関する経験共有、データ分析、AI アドバイスを得られるコミュニティスペースを提供します。
...
**선과지담**(Xuan Ke Zhi Tan, 选科知谈)은 중국의 신가오카오(新高考) 과목 선택을 지원하는 풀스택 포럼 애플리케이션입니다. 고등학교 1학년 학생과 학부모가 과목 선택 경험을 공유하고, 데이터 기반 인사이트를 탐색하며, AI 기반 맞춤형 조언을 받을 수 있는 커뮤니티 공간을 제공합니다.
